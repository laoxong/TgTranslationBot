...
//翻译
function fanyi($text,$form='auto',$to='zh-CN')
{
...
function TG_BOT_LOG($sendmessage){
	...
	$data_file=ROOT_PATH.'/log/'. date('y-m-d',time()) .'.log.txt';//log文件
	...
	$log=[
		'type'=>$update['message']['chat']['type'],//聊天类型
		'group'=>[
			'name'=>$group_title,
			'uid'=>$group_username,//群组ID
			],
		'user'=>[
			'name'=>$update['message']['from']['first_name'],//用户昵称
